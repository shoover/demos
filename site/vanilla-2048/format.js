...
// that length the panel is narrower than the line on anything but a wide desktop.
const UNITS = [
  [1e9, "B"],
  [1e6, "M"],
  [1e3, "k"],
...
// Three significant figures: a decimal place while the scaled number is under 100 --
// "10.1k" -- and none once it is past, where "123k" already says everything the reading
// is good for and the decimal only costs the line two characters. Trailing zeros are
// dropped at either width, so "10M" never reads as "10.0M".
const scale = (value, threshold) => {
...
export const abbreviate = (value) => {
  for (let i = 0; i < UNITS.length; i++) {
    ...
    const scaled = scale(value, threshold);
    ...
    if (scaled >= 1000 && i > 0) {
      ...
      return `${scale(value, biggerThreshold)}${biggerSuffix}`;
    }
